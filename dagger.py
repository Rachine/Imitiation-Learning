...
class DAgger(object):
    ...
    def build_initial_dataset(self):
        ...
        for idx in range(num_words): 
            for jdx in range(1,len(self.words[idx])): 
                ...
                self.seen_states.append((idx,jdx,self.sequences[idx][jdx-1]))
    ...
    def run(self, N = 10):
        ...
        test_fold = 9
        hat_policies = []
        X_train = sum([self.dataset[i] for i in self.dataset.keys() if i in [1]], [])
        y_train = sum([self.labels[i] for i in self.labels.keys() if i in [1]], [])
