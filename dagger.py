    ...
    def build_initial_dataset(self):
        ...
        for idx in range(num_words): 
            for jdx in range(1, len(self.words[idx])):
                ...
                self.seen_states.add((idx, jdx, self.sequences[idx][jdx-1]))
        ...
        for idx in range(num_words):
            if self.words_fold[idx] != test_fold:
                    for jdx in range(1, len(self.words[idx])):
                        ...
                        if jdx != (len(self.words[idx])-1) and (idx, jdx+1, y_pred[0]) not in self.seen_states:
                            ...
                            self.seen_states.add((idx, jdx, y_pred[0]))
